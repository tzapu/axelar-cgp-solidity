...
const methodDecoders = {
  deployToken(calldata) {
    const [name, symbol, decimals, cap, address] = defaultAbiCoder.decode(
      ['string', 'string', 'uint8', 'uint256', 'address'],
      calldata,
    );

    return { name, symbol, decimals, cap: cap.toString(), address };
  },
  mintToken(calldata) {
  ...
  .then((txs) =>
    txs.map(({ blockNumber, hash: txHash, data, status, logs }) => {
      try {
        ...
        const [chainId, signersRole, commandIds, commands, methodCalldatas] =
        ...
        return {
          ...
          txHash,
          signers,
          signersRole: signersRole.toNumber(),
          successful: status === 1,
